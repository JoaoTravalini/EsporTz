...
export const getHighlights = asyncHandler(async (req, res) => {
    const { sport, featured, limit = 20, offset = 0, sortBy = 'createdAt', order = 'DESC' } = req.query;
    ...
    queryBuilder
        .orderBy(`highlight.${sortBy}`, order)
        .limit(Number(limit))
...
export const toggleLikeHighlight = asyncHandler(async (req, res) => {
    const highlightId = req.params.id;
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: "Unauthorized" });
...
export const getFeaturedHighlights = asyncHandler(async (req, res) => {
...
// Search highlights
export const searchHighlights = asyncHandler(async (req, res) => {
    ...
    const queryBuilder = highlightRepository
        ...
        .leftJoinAndSelect("highlight.likedBy", "likedBy")
        .where("highlight.isPublic = :isPublic", { isPublic: true })
        .andWhere("(LOWER(highlight.title) LIKE LOWER(:query) OR LOWER(highlight.description) LIKE LOWER(:query))", { query: `%${q}%` });
    if (sport) {
        queryBuilder.andWhere("sport.name = :sport", { sport });
    }
