...
export const getTacticalAnalyses = asyncHandler(async (req: Request, res: Response) => {
    const {
        sport,
        author,
        featured,
    ...
    queryBuilder
        .orderBy(`analysis.${sortBy as string}`, order as 'ASC' | 'DESC')
        .limit(Number(limit))
...
export const getTacticalAnalysisById = asyncHandler(async (req: Request, res: Response) => {
...
export const createTacticalAnalysis = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    if (formation) analysisData.formation = formation;
    if (tacticalPatterns) analysisData.tacticalPatterns = tacticalPatterns;
    if (keyMoments) analysisData.keyMoments = keyMoments;
    if (statistics) analysisData.statistics = statistics;
    if (aiInsights) analysisData.aiInsights = aiInsights;
    if (highlight) analysisData.highlight = highlight;
    if (match) analysisData.match = match;
...
export const updateTacticalAnalysis = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const analysisId = req.params.id;
    const userId = req.user?.id;
...
export const deleteTacticalAnalysis = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const analysisId = req.params.id;
    const userId = req.user?.id;
...
export const toggleLikeAnalysis = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const analysisId = req.params.id;
    const userId = req.user?.id;
...
export const addComment = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    let parentComment: TacticalComment | null = null;
    if (parentCommentId) {
...
export const getUserTacticalAnalyses = asyncHandler(async (req: Request, res: Response) => {
...
export const searchTacticalAnalyses = asyncHandler(async (req: Request, res: Response) => {
    ...
    const queryBuilder = tacticalRepository
        ...
        .where("analysis.isPublic = :isPublic", { isPublic: true })
        .andWhere(
            "(LOWER(analysis.title) LIKE LOWER(:query) OR LOWER(analysis.content) LIKE LOWER(:query))",
            { query: `%${q}%` }
        );
